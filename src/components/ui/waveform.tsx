...
export const AudioWaveform = ({
  ...
  useEffect(() => {
    ...
    return () => {
      // Don't disconnect on cleanup as the audio element might be reused
    }
  }, [audioRef])
  ...
  useEffect(() => {
    ...
    const animate = () => {
      ...
      // Get frequency data if playing and analyser is connected
      let frequencyValue = 0.1
      if (playing && analyserRef.current && audioContextRef.current) {
        ...
        frequencyValue = Math.max(0.1, Math.min(1, sum / dataArray.length / 128))
      }
      ...
      if (playing) {
        ...
        barsRef.current = barsRef.current.filter((bar) => bar.x + barWidth > 0)
        ...
        while (
          ...
          if (barsRef.current.length > 200) break
        }
      } else if (barsRef.current.length === 0) {
        // Initialize with static bars when not playing
        let currentX = 0
      ...
      for (const bar of barsRef.current) {
        if (bar.x < rect.width && bar.x + barWidth > 0) {
          const barHeight = Math.max(baseBarHeight, bar.height * rect.height * 0.7)
          const y = centerY - barHeight / 2

          ctx.fillStyle = computedBarColor
          ctx.globalAlpha = playing ? 0.3 + bar.height * 0.7 : 0.3
